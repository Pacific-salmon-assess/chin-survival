...
model{
    ...
    beta_fs ~ normal( 0.25 , 0.5 );
    beta_ls ~ normal( 0.25 , 0.5 );
    ...
    for ( i in 1:N ) {
        logit_phi[i] = alpha_bar + alpha_pop[pop_n[i], 4] + alpha_yr[yr[i], 3] + beta_ds * samp_date[i] + beta_fs * size[i] + beta_ls * lipid[i] + beta_cyer * cyer[i];
...
generated quantities{
    ...
    for (i in 1:N) {
      real phi = inv_logit(alpha_bar + alpha_pop[pop_n[i], 4] + alpha_yr[yr[i], 3] + beta_ds * samp_date[i] + beta_fs * size[i] + beta_ls * lipid[i] + beta_cyer * cyer[i]);
