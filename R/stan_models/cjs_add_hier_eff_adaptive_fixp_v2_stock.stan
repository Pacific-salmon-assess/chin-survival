...
// Adds random intercepts for stock identity
...
data {
  ...
  int<lower=1,upper=nstock> year[nind];     // Stock
...
parameters {
  ...
  matrix[n_occ_minus_1, nstock] alpha_stk_phi_z;    // Stock/time random int for phi; note z so inverted before transformation
  vector<lower=0>[n_occ_minus_1] sigma_alpha_stk_phi;   // SD among stocks
  cholesky_factor_corr[n_occ_minus_1] L_Rho_stk;    // for covariance among stock-stage intercepts
