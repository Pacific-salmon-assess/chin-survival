// Equivalent to obs_surv_jll but no RIs
data{
...
model{
    ...
    for ( i in 1:N ) {
        logit_phi[i] = alpha_bar + beta_ds * samp_date[i] + beta_fs * size[i] + beta_ls * lipid[i] + beta_cyer * cyer[i] + (beta_d_cyer * samp_date[i] * cyer[i]);
    ...
    for ( i in 1:N ) {
        mu_lipid[i] = beta_dl * samp_date[i];
    }
    
    for ( i in 1:N ) {
        mu_size[i] = beta_df * samp_date[i];
    }
