...
model{
    ...
    for ( i in 1:N ) {
        logit_phi[i] = alpha_bar + alpha_pop[pop_n[i], 4] + beta_ds * samp_date[i] + beta_fs * size[i] + beta_ls * lipid[i] + beta_cyer * cyer[i] + (beta_d_cyer * samp_date[i] * cyer[i]);
...
generated quantities{
    ...
    for (i in 1:N) {
      real phi = inv_logit(alpha_bar + alpha_pop[pop_n[i], 4] + beta_ds * samp_date[i] + beta_fs * size[i] + beta_ls * lipid[i] + beta_cyer * cyer[i] + (beta_d_cyer * samp_date[i] * cyer[i]));
