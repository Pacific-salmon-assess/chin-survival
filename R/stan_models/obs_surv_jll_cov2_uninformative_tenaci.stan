...
// adjusted from obs_surv_jll_cov2_uninformative to add tenacibaculum effects and drop injury effects (too few obs)
data{
...
model{
    ...
    for ( i in 1:N ) {
        logit_phi[i] = alpha_s + alpha_stk[stk_n[i], 4] + alpha_yr[yr[i], 3] + beta_ds * day_z[i] + beta_fs * fl_z[i] + beta_ls * lipid_z[i] + beta_cs * cyer_z[i] + (beta_ds_cs * day_z[i] * cyer_z[i]) + beta_ts * tenaci_z[i];
...
generated quantities{
    ...
    for (i in 1:N) {
      real phi = inv_logit(alpha_s + alpha_stk[stk_n[i], 4] + alpha_yr[yr[i], 3] + beta_ds * day_z[i] + beta_fs * fl_z[i] + beta_ls * lipid_z[i] + beta_cs * cyer_z[i] + (beta_ds_cs * day_z[i] * cyer_z[i]) + beta_ts * tenaci_z[i]);
