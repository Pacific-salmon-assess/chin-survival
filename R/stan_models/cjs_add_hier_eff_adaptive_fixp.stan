...
parameters {
  ...
  cholesky_factor_corr[n_occ_minus_1] L_Rho_year;    // for covariance among years
  real alpha_p;                              // Mean det prob
  matrix[nyear, n_occ_minus_1 - 1] beta_p;        // Year/time intercepts for p (truncated relative to phi because last observation fixed in transformed parameters block)
}
...
transformed parameters {
  ...
  for (i in 1:nind) {
    ...
    for (t in first[i]:(n_occ_minus_1 - 1)) {
      p[i, t] = inv_logit(beta_p[year[i], t]);
    }
...
generated quantities {
  ...
  for (yy in 1:nyear) {
    ...
    for (tt in 1:n_occ_minus_1 - 1) {
      p_year[yy, tt] = inv_logit(beta_p[yy, tt]);
    }
    p_year[yy, n_occ_minus_1] = final_fix_p;
  }
