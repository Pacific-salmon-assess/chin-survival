...
parameters {
  ...
  matrix[g, n_occ_minus_1] beta_p;        // Group/time effect phi
}
...
transformed parameters {
  ...
  for (i in 1:nind) {
    ...
    // estimate phi for all time steps
    for (t in first[i]:n_occ_minus_1) {
      phi[i, t] = inv_logit(beta_phi[group[i], t]);
    }
...
generated quantities {
  ...
  for (gg in 1:g) {
    for (t in 1:n_occ_minus_1) {
      phi_g[gg, t] = inv_logit(beta_phi[gg, t]);
    }
