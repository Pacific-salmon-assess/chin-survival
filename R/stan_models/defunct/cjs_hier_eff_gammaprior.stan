...
transformed parameters {
  ...
  for (i in 1:nind) {
    ...
    for (t in first[i]:n_occ_minus_1) {
      phi[i, t] = inv_logit(mu_phi + beta_phi[group[i], t]);
      p[i, t] = inv_logit(mu_p + beta_p[group[i], t]);
    }
  }
...
generated quantities {
  ...
  for (gg in 1:g) {
    for (t in 1:n_occ_minus_1) {
      phi_g[gg, t] = inv_logit(mu_phi + beta_phi[gg, t]);
      p_g[gg, t] = inv_logit(mu_p + beta_p[gg, t]);
    }
  }
}
