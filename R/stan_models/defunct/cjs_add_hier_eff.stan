...
parameters {
  ...
  real mu_p;                              // Mean det prob
  vector[n_occ_minus_1] gamma_phi;        // Time effect phi
  // vector[g] beta_phi;                     // Group effect phi
  matrix[g, n_occ_minus_1] beta_phi;      // Group intercepts for phi that vary among stages
  real sigma_beta_phi;                    // SD among groups
  matrix[g, n_occ_minus_1] beta_p;        // Group/time interacting effect p 
}
...
transformed parameters {
  ...
  for (i in 1:nind) {
    ...
    for (t in first[i]:n_occ_minus_1) {
      phi[i, t] = inv_logit(mu_phi + (sigma_beta_phi * beta_phi[group[i], t]) + gamma_phi[t]);
...
model {
  ...
  for(t in 1:(n_occ_minus_1 - 1)) {
    for(gg in 1:g) {
      beta_p[gg, t] ~ normal(0, 1);
      beta_phi[gg, t] ~ normal(0, 1);
    }
  }
  mu_phi ~ normal(0.3, 1.5);
  mu_p ~ normal(0, 1.5); 
  gamma_phi ~ normal(0, 1);
  // logit variance so not constrained to be >0
  sigma_beta_phi ~ student_t(3, 0, 1);
...
generated quantities {
  ...
  for (gg in 1:g) {
    for (t in 1:n_occ_minus_1) {
      phi_g[gg, t] = inv_logit(mu_phi + (sigma_beta_phi * beta_phi[gg, t]) + gamma_phi[t]);
