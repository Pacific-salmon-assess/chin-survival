...
// Based on cjs_add.stan from example-models repo, but includes hierarchical time-varying effects for phi and p
...
parameters {
  ...
  vector<lower=0>[n_occ_minus_1] sigma_phi;   // SD among stock stage-specific
                                              // survival
  vector<lower=0>[n_occ_minus_1] sigma_p;     // SD among stock stage-specific
                                              // det
}
...
transformed parameters {
  ...
  for (i in 1:nind) {
    ...
    for (t in first[i]:n_occ_minus_1) {
      phi[i, t] = inv_logit(mu_phi + gamma_phi[t] + beta_phi[group[i], t]);
      p[i, t] = inv_logit(mu_p + gamma_pp[t] + beta_p[group[i], t]);
    }
  }
...
model {
  ...
  for(t in 1:n_occ_minus_1) {
    beta_phi[ , t] ~ normal(0, sigma_phi[t]);
    beta_p[ , t] ~ normal(0, sigma_p[t]);
  }
