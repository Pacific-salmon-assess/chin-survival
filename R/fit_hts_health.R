### Hierarchical Terminal Survival Model
## Estimates terminal survival conditional (i.e. detection probability not 
## estimated) based on causal pathways represented by DAG
## Repeat analysis with subset of individuals with FIT chip data for A Bass 
## analysis
...
det_dat <- det_dat1 %>% 
  ...
  mutate(
    ...
    tenaci_z = scale(tenaci_load) %>% as.numeric(),
    year = as.factor(year),
    stock_group = factor(
      stock_group, 
      levels = c(
...
dat_list <- list(
  ...
  tenaci_z = det_dat$tenaci_z
)


mod1 <- stan_model(
  here::here("R", "stan_models", "obs_surv_jll_cov2_uninformative_tenaci.stan")
  )

m1_stan_adj <- sampling(
  mod1, data = dat_list,
...
saveRDS(
  m1_stan_adj,
  here::here("data", "model_outputs", 
             "hier_binomial_cyer_stan_uninformative_adj_tenaci.rds")
)

m1_stan_adj <- readRDS(
  here::here(
    "data", "model_outputs", "hier_binomial_cyer_stan_uninformative_adj_tenaci.rds")
  )
...
# Parameters with Rhat > 1.01 (common threshold)
summ %>% 
  filter(rhat > 1.01)
...
## focus on tenaci effect
tenaci_seq <- seq(-0.2, 7.6, length = 40)
pred_t <- sapply(
  tenaci_seq, 
  function (x) {
    inv_logit(post$alpha_s + post$beta_ts * x)
  }
)
pred_tenaci_ribbon <- pred_t %>% 
  as.data.frame() %>% 
  set_names(tenaci_seq) %>% 
  pivot_longer(
    cols = everything(), names_to = "tenaci_z", values_to = "est"
  ) %>% 
  mutate(
    tenaci_z = as.numeric(tenaci_z),
    tenaci_load = (tenaci_z * sd(det_dat$tenaci_load)) + mean(det_dat$tenaci_load)
  ) %>% 
  group_by(tenaci_load) %>% 
  summarize(
    med = median(est),
  ...
  labs(y = "Predicted Survival Rate", x = "Tenaci Load") +
  ggsidekick::theme_sleek() +
...
# posterior estimate
tenaci_post <- data.frame(
  post_est = post$beta_ts
) 
tenaci_hist <- ggplot() +
  geom_histogram(data = tenaci_post, aes(x = post_est), 
                 bins = 50, fill = "#7570b3") +
  geom_vline(aes(xintercept = 0), lty = 2 , colour = "black", linewidth = 1) +
  ggsidekick::theme_sleek() +
  labs(x = "Posterior Estimate of Tenacibaculum\nLoad Effect Size") +
  theme(
    axis.title.y = element_blank()
  ) 


png(here::here("figs", "hts", "tenaci_cond_effect.png"),
    units = "in", res = 250, height = 4, width = 4.5)
pred_tenaci_ribbon
dev.off()

png(here::here("figs", "hts", "tenaci_posterior.png"),
    units = "in", res = 250, height = 4, width = 4.5)
tenaci_hist
dev.off()


## All other parameters same as fit_hts script
