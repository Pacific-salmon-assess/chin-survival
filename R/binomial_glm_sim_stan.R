...
# correlated versions
yr_rho <- 0.3
yr_mu <- c(0, 0, 0) #average effects both centered on zero
yr_sigmas <- c(0.4, 0.2, 0.3)
yr_Rho <- diag(1, 3) + matrix(yr_rho, 3, 3) - diag(yr_rho, 3)
yr_Sigma <- diag(yr_sigmas) %*% yr_Rho %*% diag(yr_sigmas)
yr_ints <- MASS::mvrnorm(n_years, yr_mu, yr_Sigma)
# alpha_yr_cond <- yr_ints[ , 1]
alpha_yr_size <- yr_ints[ , 1]
alpha_yr_lipid <- yr_ints[ , 2]
alpha_yr_surv <- yr_ints[ , 3]
...
mu_size <- mu_lipid <- rep(NA, times = nrow(dat))
...
dat$samp_date <- dat$samp_date %>% scale() %>% as.numeric()
...
size_lipid_mat <- MASS::mvrnorm(
  n = nrow(dat), mu = c(0, 0), Sigma = Sigma_cond
  ) + cbind(mu_size, mu_lipid)
dat$size <- size_lipid_mat[ , 1] %>% scale() %>% as.numeric()
dat$lipid <- size_lipid_mat[ , 2] %>% scale() %>% as.numeric()
cor(dat$size, dat$lipid)
...
true_pars <- data.frame(
  ...
  true = c(beta_dl, beta_df, surv_bar, beta_ds, 
           beta_cyer, beta_fs, beta_ls, beta_d_cyer
           )
)
...
# initial prior check
n_draws <- 1000
alpha_bar_draws <- rnorm(n_draws, 0.5, 1)
beta_ds_draws  <- rnorm(n_draws, 0.5, 0.75)
beta_cs_draws  <- rnorm(n_draws, 0.5, 0.75)


samp_date_vals <- c(-2, 0, 2)
cond_vals      <- c(-2, 0, 2)

# Create a list to store predictions
pred_list <- list()

# Loop over each combination of cond and samp_date
for (cond_val in cond_vals) {
  for (sd in samp_date_vals) {
    # Compute the linear predictor: lp = alpha_bar + beta_ds * samp_date + beta_cs * cond
    lp <- alpha_bar_draws + beta_ds_draws * sd + beta_cs_draws * cond_val
    # Convert to survival probability via the logistic function
    p_pred <- plogis(lp)
    ...
    pred_list[[length(pred_list) + 1]] <- df_temp
  }
}
...
# Convert samp_date and cond to factors (optional, for plotting clarity)
pred_df <- pred_df %>%
  mutate(
    samp_date = factor(samp_date, levels = c(-2, 0, 2)),
    cond = factor(cond, levels = c(-2, 0, 2))
  )

# Create density plots faceted by cond
ggplot(pred_df, aes(x = p, fill = samp_date)) +
...
# mod2 <- stan_model(here::here("R", "stan_models", "obs_surv_jll2.stan"))
# m2_stan <- sampling(mod2, data = dat_list,
#                     chains = 4, iter = 2000, warmup = 1000,
#                     control = list(adapt_delta = 0.95))


# observed survival model with covariance
