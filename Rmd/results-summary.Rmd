...
```{r mean-det, echo=FALSE, fig.cap = "Mean proportion of tagged population observed at each stage of the migration by tagging year. The number and description of migration stages differs among stocks, corresponding to the groups of arrays defined in Figure 2. We excluded ECVI and North Puget Sound stocks because too few individuals were tagged to reliably fit stock-specific CJS models."}
...
```{r logistic-dag, echo=FALSE, fig.cap = "Directed acyclic graphs representing hypothesized causal paths among covariates influencing indivdual survival."}
library(dagitty)
online_dag <- dagitty('
dag {
...
4. What should the response variable in the binomial GLM be?
  + Intermediate survival: 0 if a fish is never detected, 1 if it is detected at all. This would minimize differences in terminal detection probability, but given that most (>90%) individuals were detected at least once there may be insufficient contrast.
  + Terminal survival: 0 if a fish is never detected, 1 if it is detected in-river (or in Puget Sound). This seems more appropriate, though stock effects will be difficult to interpret given differences in terminal detection probability. Could resolve by excluding stocks with low terminal $p$ or focusing on stock identity as a confounding variable, rather than a variable of interest itself.
