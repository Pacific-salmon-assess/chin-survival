...
We fit CJS models in a Bayesian framework with weakly informative priors. We confirmed models sampled adequately (e.g., chain plots, $\hat{R}$, $n_{eff}$) and generated reasonable estimates with posterior predictive checks. We drew inference on stage-specific survival using two posterior estimates. The first are posterior estimates of $\phi_s$. Relatively similar estimates of $\phi_s$ suggest a similar proportion of the tagged population dies in each stage of the migration, while variable estimates suggest evidence of mortality bottlenecks. Second we calculated the cumulative product of $\phi_s$ to estimate total survival throughout the migration. Note that we did not account for known harvest in the CJS model so survival reflects the impacts of tagging, natural, and harvest mortality.
